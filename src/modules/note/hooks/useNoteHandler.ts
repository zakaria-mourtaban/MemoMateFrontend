...
export const useNoteHandlers = () => {
  ...
  useEffect(() => {
    ...
    if (current._id) {
      apiCall("GET", `api/workspace/${current._id}`, {}, true)
        .then((res) => {
          const data = res.data.workspace.files;
          setTreeData(data.map((e) => ({
            id: e._id,
            name: e.name,
            children: e.children,
          })));
  ...
  const createDiagram = useCallback(async () => {
    ...
    if (result.isConfirmed && result.value) {
      try {
        const res = await apiCall("POST", "api/chat/diagram", { prompt: result.value }, true);
  ...
  const indexCaller = useCallback((index: number) => {
    switch (index) {
      case 0:
        createDiagram();
        break;
